...
const DEFAULT_POSTGRES_POOL_IDLE_TIMEOUT_MS = 30_000;
const DEFAULT_POSTGRES_POOL_WAITING_LOG_INTERVAL_MS = 60_000;
const DEFAULT_POSTGRES_POOL_ACQUIRE_TIMEOUT_MS = 10_000;
...
/**
 * Minimal pool surface observed by Panda. Keep tests and callers on this
 * structural seam instead of pretending every fake is a full `pg.Pool`.
 */
...
/**
 * Resolves Panda's runtime database URL from explicit input or `DATABASE_URL`.
 */
...
/**
 * Returns the database URL or throws the standard operator-facing error.
 */
...
/**
 * Creates a standard `pg.Pool` with Panda's default pool limits.
 */
...
export function observePostgresPool(options: PostgresPoolObserverOptions): PostgresPoolObserver {
  ...
  const originalConnect = options.pool.connect.bind(options.pool);
  const originalQuery = options.pool.query.bind(options.pool);
  ...
  }) as ObservablePostgresPool["connect"];
  ...
  }) as ObservablePostgresPool["query"];
