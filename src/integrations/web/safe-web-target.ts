...
type LookupFamily = number | "IPv4" | "IPv6";
type PinnedLookupOptions = {all?: boolean; family?: LookupFamily};
...
export type PinnedLookup = (
  hostname: string,
  options: number | PinnedLookupOptions | LookupCallback | undefined,
  callback?: LookupCallback,
) => void;

const defaultPinnedLookupFallback: PinnedLookup = (hostname, options, callback) => {
  ...
  if (typeof options === "function" || options === undefined) {
    dnsLookupCb(hostname, cb);
    return;
  }
  if (typeof options === "number") {
    dnsLookupCb(hostname, options, cb);
    return;
  }
...
interface SafeHttpTargetOptions {
...
function normalizeAllowedProtocol(protocol: string): string | undefined {
  const normalized = trimToUndefined(protocol)?.toLowerCase();
  if (!normalized) {
    return undefined;
  }
...
/** Resolves hostnames through the system resolver while preserving all returned addresses for SSRF checks. */
...
async function resolveAddresses(
  ...
  const addresses = results
    .map((address) => trimToUndefined(address))
...
/** Creates a Node lookup callback pinned to addresses that already passed safe-target validation. */
export function createPinnedLookup(params: {
  ...
  const fallback = params.fallback ?? defaultPinnedLookupFallback;
  ...
  const lookup: PinnedLookup = (host, options, callback) => {
    ...
    const opts =
      typeof options === "object" && options !== null
        ? (options as PinnedLookupOptions)
        : {};
...
/** Validates an HTTP target and returns a DNS-pinned lookup for the approved hostname. */
