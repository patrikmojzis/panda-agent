...
import {filterRemoteShellEnv} from "./bash-remote-env.js";
import type {
    BashExecutionResult,
...
export interface BashExecutor {
  execute(options: BashExecutorOptions): Promise<BashExecutionResult>;
}
...
function readAgentKey(context: PandaSessionContext | undefined): string {
...
export function buildRemoteShellEnv(invocationEnv: Record<string, string> | undefined): Record<string, string> {
  // Remote runners own their base env. Core only forwards one-shot per-call
  // values so host env and prior shell exports never bleed into the container.
  return filterRemoteShellEnv(invocationEnv);
}
...
export class LocalShellExecutor implements BashExecutor {
  ...
  async execute(options: BashExecutorOptions): Promise<BashExecutionResult> {
    ...
    const outcome = await executeBashCommand({
      ...
      noOutputExpected: options.noOutputExpected,
  ...
  private async sendExecRequest(
    ...
    const response = await this.fetchImpl(buildRunnerEndpoint(runnerUrl, "exec"), {
      ...
      body: JSON.stringify({
        ...
        noOutputExpected: options.noOutputExpected,
        maxOutputChars: options.maxOutputChars,
        env: buildRemoteShellEnv(options.env),
