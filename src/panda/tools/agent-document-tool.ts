...
function readScope(context: unknown): Scope {
  ...
  return {
    ...
    timeZone: resolveDateTimeContextOptions().timeZone,
  };
}
...
export class AgentDocumentTool<TContext = DefaultAgentSessionContext>
  ...
  static schema = z.object({
    ...
    date: z.string().trim().optional().describe("Only for diary. Defaults to the current local day in YYYY-MM-DD using the current system timezone."),
