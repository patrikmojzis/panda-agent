...
const READONLY_VIEW_GUIDANCE = [
  ...
  "The readonly tool already scopes session.threads, session.messages, session.tool_results, session.inputs, session.runs, session.todos, session.subagent_history, session.scheduled_tasks, session.scheduled_task_runs, session.watches, session.watch_runs, and session.watch_events to the current session.",
  "The readonly tool also scopes session.prompts to the current session, and session.agent_pairings and session.agent_skills to the current agent.",
