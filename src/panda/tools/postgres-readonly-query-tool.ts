...
const READONLY_VIEW_GUIDANCE = [
  ...
  "The readonly tool also scopes session.agent_prompts, session.agent_pairings, session.agent_skills, and session.agent_telepathy_devices to the current agent.",
  "Do not invent is_active flags or extra session_id subqueries unless you are joining raw tables outside the session.* views.",
  "Use session.agent_prompts for agent docs, session.agent_pairings for known identities, session.agent_skills for stored skills, and session.agent_telepathy_devices for registered Mac receivers.",
