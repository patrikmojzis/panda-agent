...
The email has already been persisted. Use \`panda email read ${options.messageId}\` for the full message, attachment ids, and storage status. Use \`panda email attachments fetch <attachment-id>\` only for attachments with storageStatus \`stored\`, or \`panda email list\` / \`panda email search\` for nearby context.
