...
import {PANDA_RUNTIME_ERROR_SUMMARY} from "../../../integrations/postgres/schema-versions/0025-runtime-error-summary.js";

export const RUNTIME_ERROR_SUMMARY_MIGRATION: PostgresMigration = {
  ...PANDA_RUNTIME_ERROR_SUMMARY,
  apply: async ({queryable}) => {
    await queryable.query('ALTER TABLE "runtime"."runs" ADD COLUMN error_summary TEXT');
    let after: string | null = null;
    for (;;) {
      const result = await queryable.query(`
        SELECT id, CASE WHEN status = 'failed' THEN error ELSE abort_reason END AS error
        FROM "runtime"."runs"
        WHERE (status = 'failed' OR (status = 'running' AND abort_reason IS NOT NULL))
          AND ($1::uuid IS NULL OR id > $1::uuid)
        ORDER BY id
        LIMIT 100
      `, [after]);
      const rows = result.rows as Array<{id: string; error: string | null}>;
      if (rows.length === 0) break;
      await queryable.query(`
        UPDATE "runtime"."runs" AS run
        SET error_summary = summary.value
        FROM unnest($1::uuid[], $2::text[]) AS summary(id, value)
        WHERE run.id = summary.id
      `, [rows.map((row) => row.id), rows.map((row) => summarizeRuntimeError(row.error))]);
      after = rows.at(-1)!.id;
    }
  },
};

// Frozen September 2026 sanitizer for deterministic historical backfill. Live writes
// use src/lib/runtime-error-summary.ts; later policy changes need an explicit backfill.
...
function summarizeRuntimeError(error: unknown, options: {maxChars?: number} = {}): string | null {
