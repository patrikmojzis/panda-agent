...
import {redactExactString, StreamingSecretRedactor} from "../src/integrations/mcp/redaction.js";
...
async function startHttpFixture(): Promise<{base: string; mcp: string; sse: string}> {
  const child = spawn(process.execPath, [fixturePath, "--transport", "http", "--port", "0"], {
    cwd: root,
...
describe("MCP SDK runner", () => {
  ...
  it.each([
    ...
    ["remote error", "/error", 500],
    ["oversized body", "/oversize", undefined],
  ])("fails closed on %s without returning response bodies or secrets", async (_label, pathname, status) => {
    const fixture = await startHttpFixture();
    let caught: unknown;
    try {
    ...
    expect(caught).toMatchObject({exitCode: 3, phase: "transport_protocol", ...(status ? {httpStatus: status} : {})});
  ...
  it("kills stdio before parsing a JSON-RPC line over 8 MiB", async () => {
    await expect(new SdkMcpRunner().listTools(invocation({
      ...
      timeoutMs: 2_000,
    }))).rejects.toMatchObject({exitCode: 3, phase: "transport_protocol"});
  });
