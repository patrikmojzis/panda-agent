...
import {parseEmailAuthenticationHeaders} from "../src/domain/email/auth.js";
...
describe("email authentication parsing", () => {
  it("keeps authentication passes unknown because raw headers are not trusted", () => {
    expect(parseEmailAuthenticationHeaders({
      authenticationResults: "mx.example; spf=pass smtp.mailfrom=example.com; dkim=pass header.d=example.com; dmarc=pass header.from=example.com",
    })).toEqual({
      authSpf: "pass",
      authDkim: "pass",
      authDmarc: "pass",
  ...
  it("marks any provider authentication failure as suspicious", () => {
    expect(parseEmailAuthenticationHeaders({
      authenticationResults: "mx.example; spf=pass smtp.mailfrom=example.com; dkim=pass header.d=example.com; dmarc=fail header.from=evil.example",
    })).toEqual({
      authSpf: "pass",
      authDkim: "pass",
      authDmarc: "fail",
  ...
  it("does not treat SPF or DKIM alone as trusted without DMARC pass", () => {
    expect(parseEmailAuthenticationHeaders({
      authenticationResults: "mx.example; spf=pass smtp.mailfrom=example.com; dkim=pass header.d=example.com",
    })).toEqual({
      authSpf: "pass",
      authDkim: "pass",
      authSummary: "unknown",
  ...
  it("marks missing provider authentication as unknown", () => {
    expect(parseEmailAuthenticationHeaders({})).toEqual({
      authSummary: "unknown",
    });
  });
});
