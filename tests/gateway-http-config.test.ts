...
describe("gateway HTTP config", () => {
  it("resolves defaults and explicit env values outside the route dispatcher", () => {
    expect(resolveGatewayHttpConfig({})).toMatchObject({
      ...
      maxConcurrentAttachmentUploads: 8,
      attachmentRequestTimeoutMs: 60_000,
    ...
    expect(resolveGatewayHttpConfig({
      ...
      GATEWAY_MAX_CONCURRENT_ATTACHMENT_UPLOADS: "3",
      GATEWAY_ATTACHMENT_REQUEST_TIMEOUT_MS: "5000",
      ...
      maxConcurrentAttachmentUploads: 3,
      attachmentRequestTimeoutMs: 5000,
