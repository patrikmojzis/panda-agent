...
import {REFRESH_ARCHIVED_SESSION_VIEW_MIGRATION} from "../src/app/database/migrations/0008-refresh-archived-session-view.js";
...
describe("session archive readonly view migration", () => {
  it("runs after legacy reconciliation and appends archived_at without changing existing columns", async () => {
    ...
    expect(ids.indexOf("0008_archive_session")).toBeLessThan(ids.indexOf("0008_legacy_schema_reconciliation"));
    expect(ids.indexOf("0008_legacy_schema_reconciliation")).toBeLessThan(ids.indexOf("0008_refresh_archived_session_view"));
    expect(ids.indexOf("0008_refresh_archived_session_view")).toBeLessThan(ids.indexOf("0009_bound_secret_envelopes"));
