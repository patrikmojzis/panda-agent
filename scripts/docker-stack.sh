...
Notes:
  ...
  - Secret-bearing env files must be owner-only (0600 or stricter).
  - permissions-fix repairs the selected env file and Panda-managed generated paths.
EOF
}
...
repair_stack_permissions() {
  local managed_file obsolete_wiki_env
  repair_private_env_file "$env_file" || exit "$?"

  if [[ -e "$generated_dir" || -L "$generated_dir" ]]; then
    ensure_private_managed_directory "$generated_dir" || exit "$?"
    printf 'Secured Panda-managed directory (0700): %s\n' "$generated_dir"

    for managed_file in \
      "$generated_compose" \
      "$generated_wiki_compose" \
      "$generated_public_caddyfile"; do
      secure_private_managed_file "$managed_file" || exit "$?"
    done

    if [[ -e "$generated_dir/wiki" || -L "$generated_dir/wiki" ]]; then
      ensure_private_managed_directory "$generated_dir/wiki" || exit "$?"
      printf 'Secured Panda-managed directory (0700): %s\n' "$generated_dir/wiki"
      secure_private_managed_file "$generated_dir/wiki/docker-compose.wiki.ssl.yml" || exit "$?"
    fi

    obsolete_wiki_env="$generated_dir/wiki-host.env"
    remove_obsolete_private_managed_file "$obsolete_wiki_env" || exit "$?"
  fi

  printf 'Permissions are ready. Re-run your stack command.\n'
  printf 'Only the selected env file and known Panda-managed generated paths were changed.\n'
  printf 'Other host files were not scanned; secure manually created secret files separately.\n'
}

command_name="${1:-}"
if [[ "$command_name" == "permissions-fix" ]]; then
  shift || true
  [[ $# -eq 0 ]] || die "permissions-fix does not take arguments."
  repair_stack_permissions
  exit 0
fi

require_private_env_file "$env_file" "./scripts/docker-stack.sh permissions-fix" || exit "$?"
...
render_generated_wiki_compose() {
  ...
  ensure_private_managed_directory "$generated_dir" || exit "$?"
  secure_private_managed_file "$generated_wiki_compose" 0 || exit "$?"
...
render_generated_public_caddyfile() {
  ...
  ensure_private_managed_directory "$generated_dir" || exit "$?"
  secure_private_managed_file "$generated_public_caddyfile" 0 || exit "$?"
...
EOF
      ...
      PANDA_CONTROL_PUBLIC_URL: \${PANDA_CONTROL_PUBLIC_URL:-}
...
bootstrap_wiki_if_configured() {
  ...
  (
    cd "$repo_root"
    WIKI_DOCKER_BIN="$docker_bin" \
      PANDA_WIKI_BINDING_TRANSPORT=compose \
      WIKI_ENV_FILE="$env_file" \
