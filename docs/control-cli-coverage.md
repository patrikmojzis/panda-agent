...
`origin/main` (`db0a2e2`). Update it whenever `src/app/cli.ts` registers a new
...
| Gateway safe setup: `gateway source create/list/allow-type/disallow-type/rotate-secret/suspend/resume`, `gateway device register/list/enable/disable`, `gateway event-list` | Gateway tab and session gateway tab in `control-routes.ts`; source/device/event-type/event APIs in `api.ts`; handlers/services in `http-server.ts` and `operator-service.ts`. | **B** covered safe setup/read flows; one-time secrets stay guarded/audited. Device token rotation is not a generic setup button. |
| Gateway device command queue and maintenance: `gateway device rotate-token`, `gateway device command enqueue/list/cancel/timeout-sweep`, `gateway attachment-scrub-expired` | No broad Control UI surface. Current Control covers gateway setup/events, not queue mutation or attachment scrubbing. | **C/A**: dangerous/admin maintenance and token rotation stay CLI-only unless #231 scopes a safer boundary; any read-only queue health belongs there too. |
