...
| `whatsapp.chat.list` | Native `panda whatsapp chat list --connector <key>`; `--json` input remains. | Done for current-session WhatsApp chat discovery from durable bindings. The account connector key is explicit; there is no process-wide WhatsApp default. | P1 |
