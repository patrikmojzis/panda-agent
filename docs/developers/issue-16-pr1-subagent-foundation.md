...
- `agent_skill` is not in `core` in PR1 because the raw tool currently supports `load`, `set`, and `delete`; adding it to universal raw membership would silently grant skill mutation. The approved long-term intent is operation-level access: load/read broadly, set/delete only for `operate` or a skill-maintainer-equivalent policy.
