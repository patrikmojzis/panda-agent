...
docker build --target bash-runner --build-arg NODE_MAJOR="${PANDA_RUNNER_NODE_MAJOR:-22}" -t panda-runner:latest .
...
- optional `panda-telegram` when `TELEGRAM_ENABLED=true`; it runs `TELEGRAM_ACCOUNT_KEY` (default `main`)
